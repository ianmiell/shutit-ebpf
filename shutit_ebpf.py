...
class shutit_ebpf(ShutItModule):
	...
	def build(self, shutit):
		...
		shutit.build['module_name'] = 'shutit_ebpf_' + ''.join(random.choice(string.ascii_letters + string.digits) for _ in range(6))
  ...
  config.vm.define "ebpf1" do |ebpf1|
    ebpf1.vm.box = ''' + '"' + vagrant_image + '"' + '''
    ebpf1.vm.hostname = "ebpf1.vagrant.test"
    config.vm.provider :virtualbox do |vb|
      vb.name = "shutit_ebpf_1"
    end
  end
end''')
		...
		machines = {}
		machines.update({'ebpf1':{'fqdn':'ebpf1.vagrant.test'}})
...
def module():
	return shutit_ebpf(
		'imiell.shutit_ebpf.shutit_ebpf', 2144390141.0001,
		description='',
		maintainer='',
